...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Match GPS track with TTN LoRaWAN uplinks and plot coverage map.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python coverage_map.py --gps walk.gpx --ttn uplinks.json
  python coverage_map.py --gps track.kml --ttn uplinks.json --max-delta 30
  python coverage_map.py --gps walk.gpx  --ttn uplinks.json --output berlin_test.html
        """
    )
    parser.add_argument('--gps',       required=True,
                        help='Path to .gpx or .kml GPS track file')
    parser.add_argument('--ttn',       required=True,
                        help='Path to TTN uplinks JSON file')
    parser.add_argument('--output',    default='coverage_map.html',
                        help='Output HTML map filename (default: coverage_map.html)')
    parser.add_argument('--max-delta', type=float, default=10.0,
                        help='Max timestamp gap in seconds for matching (default: 10)')
...
def main():
    ...
    # ── Load GPS track ────────────────────────────────────────────────────────
    gps_path = args.gps.lower()
    if gps_path.endswith('.gpx'):
        gps_df = load_gpx(args.gps)
    elif gps_path.endswith('.kml'):
        gps_df = load_kml(args.gps)
    else:
        print(f"Error: unsupported GPS format. Use .gpx or .kml", file=sys.stderr)
        sys.exit(1)

    # ── Load TTN uplinks ──────────────────────────────────────────────────────
    ttn_df = load_ttn(args.ttn)
    ...
    # ── Match ─────────────────────────────────────────────────────────────────
    matched = match_gps_to_uplinks(gps_df, ttn_df,
                                   max_delta_s=args.max_delta)

    # ── Print summary ─────────────────────────────────────────────────────────
    print(f"\n── Match Summary ──────────────────────────────────────")
    print(f"  Total uplinks in TTN file : {len(ttn_df)}")
    print(f"  Successfully matched      : {len(matched)}")
    print(f"  Match rate                : "
          f"{len(matched)/len(ttn_df)*100:.1f}%")
    if not matched['rssi'].isna().all():
        print(f"  RSSI  mean / min / max    : "
              f"{matched['rssi'].mean():.1f} / "
              f"{matched['rssi'].min():.1f} / "
              f"{matched['rssi'].max():.1f}  dBm")
        print(f"  SNR   mean / min / max    : "
              f"{matched['snr'].mean():.1f} / "
              f"{matched['snr'].min():.1f} / "
              f"{matched['snr'].max():.1f}  dB")
    print(f"───────────────────────────────────────────────────────\n")

    # ── Build map ─────────────────────────────────────────────────────────────
    build_map(gps_df, matched, args.output)

    # ── Export CSV ────────────────────────────────────────────────────────────
    if not args.no_csv:
        export_csv(matched, args.output)
