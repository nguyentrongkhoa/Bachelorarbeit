...
\acrfull{lora} ist ein von der Firma Semtech entwickeltes Modulationsverfahren und bildet die physikalische Grundlage des 
\acrshort{lorawan}"=Protokolls. Da LoRa und LoRaWAN nicht unhäufig miteinander verwechselt werden, besteht die Notwendigkeit, auf ihre Unterschiede hinzuweisen: 
LoRa bezeichnet die reine Modulationstechnologie, währen
der von der LoRa Alliance definiert und gepflegt wird~% TODO: \cite{semtech_lora_basics}. 

LoRa basiert auf dem Verfahren der \acrfull{css}"=Modulation.
...
die Bandbreite (\acrshort{bw}) sowie die Coding Rate (\acrshort{cr}).
...
dar, da die Luftzeit und damit der Energieverbrauch minimiert werden~% TODO: \cite{semtech_lora_basics}.
...
diese über eine Ethernet- oder Mobilfunkverbindung an die Netzwerkserver weiter. Umgekehrt gilt für die Downlink-Richtung, 
dass Gateways die Signale der Server Endgeräten zur Verfügung stellen. Da Gateways permanent auf allen Kanälen lauschen, 
können Endgeräte jederzeit senden, solange regulatorische Gegebenheiten es erlauben. % TODO: write about indoor and outdoor gateways
...
Zu den Aufgaben eines Network-Servers gehören: Überprüfung von Endgerätadressen, Paketsauthentifizierung, Anpassung der Datenraten
(hierauf wird im Abschnitt \ref{subsec:adr} näher eingegangen), Weiterleiten von Payloads bzw. Join-Anfragen an entsprechende 
Application-Servers und Join-Server jeweils. Zusätzlich kümmern sich Network-Server um die Konsolidierung von Nachrichten, 
die von mehreren Gateways empfangen wurden, sowie um die Überwachung der Netzwerk- und Gateway"=Leistung. % TODO: 

\subsection{Join-Server}

Der \textbf{Join Server} verwaltet die kryptografischen Wurzelschlüssel und wird
insbesondere beim \acrlong{otaa}"=Aktivierungsverfahren benötigt. Er muss in einer
...
\acrlong{otaa} und \acrlong{abp}.

Bei \acrshort{abp} werden alle benötigten Parameter -- \texttt{DevEUI}, \texttt{DevAddr}
...
Bei OTAA initiiert das Gerät beim ersten Start eine \textit{Join Request},
woraufhin der Network"= bzw. Join"=Server einen \textit{Join Accept} zurücksendet
und dabei die Sitzungsschlüssel dynamisch ableitet. OTAA ist das empfohlene
...
\acrlong{adr} ist ein Mechanismus des Network Servers zur automatischen Anpassung von
...
SF automatisch~\cite{semtech_lora_basics}. ADR ist ausschließlich für stationäre
Geräte geeignet -- für mobile Anwendungen wie das hier betrachtete Schutzsensorsystem % TODO: remove --
...
Verifikation der Firmware"=Authentizität -- wodurch das Risiko eines unbefugten % TODO: remove --
...
-- etwa über BLE oder NFC -- stellt sicher, dass Produktionsmitarbeiter keinen Zugriff % TODO: remove --
...
\acrshort{dguv} sowie die ATEX"=Richtlinie 2014/34/EU für Geräte in
